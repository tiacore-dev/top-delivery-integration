    ...
    try:
        ...
        if not data or 'auth' not in data or 'shipment_id' not in data or 'order_ids' not in data:
            raise DataValidationError(
                "auth, shipment_id и order_ids обязательны")
        auth_data = data['auth']
        shipment_id = data['shipment_id']
...
def set_orders_sent_to_delivery():
    try:
        ...
        response = td.set_sent_to_delivery(
            auth_data, order_id, date_sent_to_delivery, delivery_type, service_id, delivery_params)
        return jsonify(response)
    except DataValidationError as e:
        raise e
    except Exception as e:
